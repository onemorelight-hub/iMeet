...
io.on('connection', (socket) => {
    ...
    socket.on('chat message', (msg) => {
        console.log("message: "+msg.id +" "+msg.name);
  ...
  //-----------------------------------------------------------------
    ...
    socket.on("callaingMedia", (msg)=>{
      console.log("callaingMedia-> socket: "+msg.id +" callaingUser: "+msg.callingUser);
    ...
    socket.on("MediaUserBusy", msg=>{
      console.log("MediaUserBusy-> socket: "+msg.id);
      io.to(msg.id).emit("MediaUserBusy",{"id": socket.id, "userName": msg.userName});
    })

    socket.on("waitingMediaReady", msg=>{
      console.log("waitingMediaReady-> socket: "+msg.id);
      io.to(msg.id).emit("waitingMediaReady",msg);
    })
    //-------------------------------------------------------------

    socket.on('message', msg=> {
      console.log("On Message-> id: "+ msg.id +" type: "+msg.description.type);
  ...
  socket.on('exitMediaCall', msg=> {
      ...
      socket.to(msg.id).emit('exitMediaCall', {"id": socket.id, "description":msg.userName});
    });
